...
#just to test the i2c_dev.jl module, I didn't break my program into
...
#***********************************************************
fd = i2c_dev.i2c_init("/dev/i2c-0", ACCEL_ADDR, I2C_SLAVE)
println("Opening device...")

#***********************************************************
#device id should be 0x1a (or 26 in decimal)
#***********************************************************
device_id = i2c_dev.i2c_smbus_read_byte_data(fd, MA8451_REG_WHOAMI)
if (device_id != 26)
...
#***********************************************************
while(true)
	ret = i2c_dev.i2c_smbus_read_byte_data(fd, MMA8451_REG_CTRL_REG2)
	if (ret < 0)
		error("Problem reading MMA8451_REG_CTRL_REG2")
		exit(ret)
	end
	if (ret == 0)
		break
...
#set resolution
#***********************************************************
i2c_dev.i2c_smbus_write_byte_data(fd, MMA8451_REG_CTRL_REG2, 0x2)
println("set resolution...")

#***********************************************************
# data ready int1
#***********************************************************
i2c_dev.i2c_smbus_write_byte_data(fd, MMA8451_REG_CTRL_REG4, 0x1)
i2c_dev.i2c_smbus_write_byte_data(fd, MMA8451_REG_CTRL_REG5, 0x1)
println("data ready int1...")

#***********************************************************
# turn on orientation
#***********************************************************
i2c_dev.i2c_smbus_write_byte_data(fd, MMA8451_REG_PL_CFG, ORIENTATION_ON)
println("turn on orientation...")

#***********************************************************
# activate at max rate, low noise mode
#***********************************************************
i2c_dev.i2c_smbus_write_byte_data(fd, MMA8451_REG_CTRL_REG1, 0x4 | ACTIVATE)
println("activate at max rate, low noise mode...")
...
#***********************************************************
reg1 = 0x0
reg1 = i2c_dev.i2c_smbus_read_byte_data(fd, MMA8451_REG_CTRL_REG1)


i2c_dev.i2c_smbus_write_byte_data(fd, MMA8451_REG_CTRL_REG1, DEACTIVATE)
i2c_dev.i2c_smbus_write_byte_data(fd, MMA8451_REG_XYZ_DATA_CFG, sensor_range)        
i2c_dev.i2c_smbus_write_byte_data(fd, MMA8451_REG_CTRL_REG1, UInt8(reg1) | ACTIVATE)                
println("first standby...")

#***********************************************************
# set the data rate for the sensor. 
#***********************************************************
data_rate = BW_RATE_400HZ
current = i2c_dev.i2c_smbus_read_byte_data(fd, MMA8451_REG_CTRL_REG1)   
println("set the data rate for the sensor....")     
...
i2c_dev.i2c_smbus_write_byte_data(fd, MMA8451_REG_CTRL_REG1, current | ACTIVATE)        
println("deactivate....")  
...
while(true)
	...
	#***********************************************************
	f_read = i2c_dev.i2c_smbus_read_byte_data(fd, MMA8451_REG_CTRL_REG1) & 2

	f_mode = i2c_dev.i2c_smbus_read_byte_data(fd, MMA8451_REG_F_SETUP) & 0xC0

	sleep(1)
	#***********************************************************
	#Read the buffer from the mma8451
	#***********************************************************
	read_bytes, buffer = i2c_dev.i2c_smbus_read_i2c_block_data(fd, MMA8451_REG_OUT_X_MSB, 0x6)  
	if (read_bytes < 0)
	...
	#***********************************************************
	println("************************************************************")
	orientation = i2c_dev.i2c_smbus_read_byte_data(fd, MMA8451_REG_PL_STATUS) & 0x7
	print("Position = ")
	println(orientation)
	print("x: ")
	println(x)
